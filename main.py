...
if __name__ == "__main__":
    ...
    state = torch.cat([state for _ in range(4)]).unsqueeze(0)
    ...
    for i in range(it, num_steps):
        prediction = model(state.to(device))[0]
        ...
        state_batch = torch.cat(tuple(s for s in state_batch)).to(device)
        action_batch = torch.from_numpy(
            np.array(
                [[0, 1] if a == 1 else [1, 0] for a in action_batch], dtype=np.float32
            )
        ).to(device)
        reward_batch = torch.from_numpy(
            np.array(reward_batch, dtype=np.float32)[:, np.newaxis]
        ).to(device)
        next_state_batch = torch.cat(tuple(s for s in next_state_batch)).to(device)
        ...
        y = torch.cat(
            tuple(
                r if d else r + gamma * torch.argmax(p)
                for r, d, p in zip(reward_batch, done_batch, next_prediction_batch)
            )
        )
